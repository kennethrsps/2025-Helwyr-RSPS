	...
	public static boolean processNormalCommand(final Player player, String[] cmd, boolean console,
		...
		if (clientCommand) {
			...
			case "mastery":
			 ...
			 case "addbot":
			        if (cmd.length < 2) {
			            player.getPackets().sendGameMessage("Usage: ::addbot [username]");
			            return true;
			        }
			        String botName = cmd[1];
			        for (int i = 2; i < cmd.length; i++) {
			            botName += " " + cmd[i];
			        }
			        BotManager.createBot(botName);
			        player.getPackets().sendGameMessage("Attempting to create bot: " + botName);
			        return true;
