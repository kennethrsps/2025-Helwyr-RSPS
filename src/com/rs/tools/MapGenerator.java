...
public class MapGenerator extends JFrame implements MouseListener, MouseMotionListener, MouseWheelListener {
	...
	public void checkLoad() {
		if (!loading) {
			...
			if (Math.abs(newRX - rxPos) >= ratio / 2 || Math.abs(newRY - ryPos) >= ratio / 2 || newMaxRX != maxRX
					|| newMaxRY != maxRY || forceNoRefresh) {
				loading = true;
				repaint();
				Thread t = new Thread() {
	...
	public void paint(Graphics g) {
		/*
		 * for(int x = 0; x < 10; x++) { for(int y = 0; y < 10; y++) { int regionId =
		 * ((x+xPos) << 8) | (y + yPos); BufferedImage[] data = getMap(regionId);
		 * if(data == null) continue; g.drawImage(data[planePos], x*64, 640-y*64, null);
		 * 
		 * } }
		 */
		...
		if (map != null && !forceNoRefresh)
			g2.drawImage(map, (int) (((int) (-localX) + (rxPos - xPos / 64) * 64 - ratio * 64) * scale),
					(int) ((-((64 + (ryPos - yPos / 64) * 64 + ratio * 64) - localY)) * scale), this);
	...
	public int[] getPos() {
		return new int[] { (int) (xPos + ratio * 64 + mouseLocation2.getX() / scale),
				(int) (yPos + ratio * 64 + (getHeight() - mouseLocation2.getY() / scale)) };
	}
		...
		for (int x = 0; x < rgb.length; x++) {
			for (int y = 0; y < rgb[0].length; y++) {
				if (((dataA[(int) (x / scale)][(int) (y / scale)]) & 0x7fff) > 0) {
					...
					int r = defs.secondaryColor != -1 ? defs.secondaryColor : defs.color;
			...
			for (int ry = 0; ry < maxRY; ry++) {
				...
				while ((incr = landStream.readSmart2()) != 0) {
					...
					while ((incr2 = landStream.readUnsignedSmart()) != 0) {
						 ...
						 * (mapSettings[1][localX][localY] & 2) == 2) objectPlane--; if (objectPlane < 0
						 * || objectPlane >= 4 || plane < 0 || plane >= 4) continue;
						 */
							...
							if (mapSpriteId != -1) {
								...
								if (spriteId != -1) {
									...
									if (width == 0 || height == 0)
										continue;
									/*
									 * if(width > s.getWidth() || height > s.getHeight()) { width = s.getWidth();
									 * height = s.getHeight();
									 * 
									 * }
									 */

									img.getGraphics().drawImage(s.getScaledInstance(width, height, 0),
											(int) ((rx * 64 + localX) * scale),
											(int) (((maxRY * 64 - 1) - (ry * 64 + localY)) * scale), null);
								}
							}
							int mapAreaId = ObjectDefinitions.getObjectDefinitions(objectId).areaId;
							if (mapAreaId != -1) {
								boolean useNewSprites = true;
								AreaDefinitions defs = AreaDefinitions.getAreaDefinitions(mapAreaId);
								if (defs != null && defs.configbyfileId == 22874 && defs.anIntArray2667 != null) {
									int spriteId = AreaDefinitions
											.getAreaDefinitions(defs.anIntArray2667[useNewSprites ? 0 : 1]).spriteId;
									if (spriteId != -1) {
										IndexedColorImageFile sprite = new IndexedColorImageFile(Cache.STORE, spriteId,
												0);
										BufferedImage s = sprite.getImages()[0];
										int width = (int) (s.getWidth() / 2 * scale);
										int height = (int) (s.getHeight() / 2 * scale);
