...
public class Masuta extends NPC {
	...
	public void handleIngoingHit(Hit hit) {
		...
		if (hit.getSource() instanceof Player) {
			...
			if (p.npcMasuta == null) {// this sets the damage and boss
				p.npcMasuta = this;
				p.npcMasutaDmg = hit.getDamage();
			}
			if (p.npcMasuta != this) {// this resets the damage
				p.npcMasuta = this;
				p.npcMasutaDmg = hit.getDamage();
			}
			if (p.npcMasuta == this && p.npcMasutaDmg < 1) {// this adds to the damage
				p.npcMasutaDmg += hit.getDamage();
			}
			if (!rewardList.contains(p) && p.npcMasutaDmg >= 1) {
				rewardList.add(p);
	...
	protected void startSpawn() {
		...
		WorldTasksManager.schedule(new WorldTask() {
			...
			public void run() {
				try {
					new Masuta(id, getRespawnTile(), getMapAreaNameHash(), canBeAttackFromOutOfArea(), isSpawned());
				} catch (Throwable e) {
					Logger.handle(e);
				}
			}
